...
emfeat_measures_default = ["peak_fluxden", "peak_redsh", "EW", "FWHM", "flux", "lum"]


def analyze_emission_feature(
    specfit,
    feature_name,
    model_names,
    rest_frame_wavelength,
    cont_model_names=None,
    redshift=None,
    dispersion=None,
    emfeat_meas=None,
    disp_range=None,
    ...
    if cont_model_names is not None:
        cont_spec = build_model_flux(specfit, cont_model_names, dispersion=dispersion)
    ...
    if cosmology is None:
        ...
        print(
            "[WARNING] No Cosmology specified. Assuming default "
    ...
    if "peak_fluxden" in emfeat_meas:
        ...
        result_dict.update(
            {
                feature_name
                + "_peak_fluxden": np.max(model_spec.fluxden) * model_spec.fluxden_unit
            }
        )
    ...
    if "peak_redsh" in emfeat_meas:
        ...
        result_dict.update(
            {
                feature_name
                + "_peak_redsh": get_peak_redshift(model_spec, rest_frame_wavelength)
            }
        )
    if "EW" in emfeat_meas and cont_model_names is not None:
        # Calculate the rest-frame equivalent width
        ew = get_equivalent_width(cont_spec, model_spec, redshift=redshift)
    ...
    if "FWHM" in emfeat_meas:
        ...
        if np.isnan(fwhm):
            print(
                "[WARNING] FWHM could not be calculated for feature "
                "{}".format(feature_name)
            )
    ...
    if "lum" in emfeat_meas:
        ...
        lum = calc_integrated_luminosity(
            model_spec, redshift=redshift, cosmology=cosmology
        )
    ...
    if "nonparam" in emfeat_meas:
        ...
        v50, v05, v10, v90, v95, v_res_at_line, freq_v50, wave_v50, z_v50 = (
            get_nonparametric_measurements(
                model_spec, rest_frame_wavelength, redshift, disp_range=disp_range
            )
        )
...
cont_measures_default = ["fluxden_avg", "Lwav", "appmag"]
...
def analyze_continuum(
    ...
    for wave in rest_frame_wavelengths:
        ...
        wave_name = str(wave)
        fluxden_avg = get_average_fluxden(
            cont_spec, wave, redshift=redshift, width=width
        )
        ...
        if "Lwav" in cont_meas:
            lwav = calc_lwav_from_fwav(
                fluxden_avg, redshift=redshift, cosmology=cosmology
            )
            result_dict.update({wave_name + "_Lwav": lwav})

        if "appmag" in cont_meas:
            appmag = calc_apparent_mag_from_fluxden(
                fluxden_avg, wave * (1.0 + redshift) * units.AA
            )
            ...
            for idx, specmodel in enumerate(specfit.specmodels):
                ...
                if set(cont_emfeat_models).issubset(prefix_list):
                    ...
                    if set(params_list_keys).issubset(mcmc_columns):
                        print(
                            "[INFO] Analyzing continuum and emission "
                            "feature {}".format(emfeat_dict["feature_name"])
                        )
                        _mcmc_analyze(
                            foldername,
                            specfit,
                    ...
                    if set(emfeat_models).issubset(prefix_list):
                        ...
                        if set(params_list_keys).issubset(mcmc_columns):
                            print(
                                "[INFO] Analyzing emission feature {}".format(
                    ...
                    if set(cont_models).issubset(prefix_list) and not cont_analyzed:
                        params_list_keys = []
                        for params in specmodel.params_list:
                            params_list_keys.extend(list(params.keys()))
                        # ONLY IF all parameters have been sampled by the MCMC
...
def _mcmc_analyze(
    ...
    for i in tqdm(range(len(mcmc_df.index))):
        ...
        fit.specmodels[specmodel_index].update_param_values_from_input_series(
            mcmc_series
        )
        ...
        if mode == "both" or mode == "emfeat":
            ...
            emfeat_result_dict = analyze_emission_feature(
                specfit,
                feature_name,
                model_names,
                rest_frame_wavelength,
        ...
        if mode == "both" or mode == "cont":
            cont_result_dict = analyze_continuum(
                specfit,
                continuum_dict["model_names"],
                continuum_dict["rest_frame_wavelengths"],
                cosmology,
        ...
        if result_table is None:
            ...
            for column in result_table.columns:
                if (
                    isinstance(result_dict[column], units.Quantity)
                    or isinstance(result_dict[column], units.IrreducibleUnit)
                    or isinstance(result_dict[column], units.CompositeUnit)
                ):
    ...
    if cont_result_dict is None:
        result.write(
            "{}/mcmc_analysis_{}.csv".format(foldername, feature_name),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
        ...
        result.write(
            "{}/mcmc_analysis_cont.csv".format(foldername),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
        ...
        result.write(
            "{}/mcmc_analysis_cont_{}.csv".format(foldername, feature_name),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
...
def analyze_resampled_results(
    ...
    * 'disp_range' - 2 element list holding the lower and upper dispersion \
    ...
    if set(all_specfit_params).issubset(fit_columns):
        ...
        print(
            "[ERROR] Resampled results do NOT contain necessary column " "information."
        )
        print("[ERROR] Please double check if the correct file was supplied.")


def _resampled_analyze(
    specfit,
    resampled_df,
    foldername,
    continuum_dict,
    ...
    for i in tqdm(range(len(resampled_df.index))):
        ...
        for emission_feature_dict in emission_feature_dictlist:
            ...
            single_emfeat_result_dict = analyze_emission_feature(
                specfit,
                feature_name,
                model_names,
                rest_frame_wavelength,
    ...
    if cont_result_dict is None and len(emission_feature_dictlist) == 1:
        result.write(
            "{}/resampled_analysis_{}.csv".format(foldername, feature_name),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
        ...
        result.write(
            "{}/resampled_analysis_lines.csv".format(foldername),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
        ...
        result.write(
            "{}/resampled_analysis_cont.csv".format(foldername),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
        ...
        result.write(
            "{}/resampled_analysis_cont_{}.csv".format(foldername, feature_name),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
        ...
        result.write(
            "{}/resampled_analysis_cont_lines.csv".format(foldername),
            format="ascii.ecsv",
            overwrite=True,
            delimiter=",",
        )
                ...
                for param in specmodel.params_list[idx]:
                    ...
                    if expr is not None:
                        ...
                        if expr in specmodel.params_list[idx]:
                            ...
                            raise ValueError(
                                "Global parameter {} not found in SpecModel".format(
                                    expr
                                )
                            )

                model_fluxden += model.eval(specmodel.params_list[idx], x=dispersion)

    model_spec = sod.SpecOneD(
        dispersion=dispersion,
        fluxden=model_fluxden,
...
def get_average_fluxden(input_spec, dispersion, width=10, redshift=0):
    ...
    disp_range = [
        (dispersion - width / 2.0) * (1.0 + redshift),
        (dispersion + width / 2.0) * (1.0 + redshift),
    ]
...
def get_equivalent_width(cont_spec, line_spec, disp_range=None, redshift=0):
    """
...
def get_fwhm(
    ...
    if method == "spline":
        spline = UnivariateSpline(
            dispersion, fluxden - np.max(fluxden) / 2.0, s=s
        )  ###s=10
        roots = spline.roots()
    elif method == "sign":
        roots_idx = np.where(np.diff(np.sign(fluxden - np.max(fluxden) / 2.0)))[0]
        roots = dispersion[roots_idx]
    else:
        raise ValueError(
            '[ERROR] Value for "method" keyword argument not '
            'recgonized. Possible values are "sign" or "spline".'
        )

    if len(roots) > 2 or len(roots) < 2:
        print("[ERROR] Found {} roots. Cannot determine FWHM".format(len(roots)))
        print("[ERROR] Setting FWHM to NaN")
        # plt.cla()
        # plt.plot(dispersion, fluxden - np.max(fluxden) / 2.0, "r-", lw=2)
        # plt.plot(dispersion, dispersion * 0, "k--")
        # plt.show()
        return np.nan
    else:
        # print("[INFO] FWHM calculated succesfully. ")
        ...
        fwhm = (abs(roots[0] - max_wav) + abs(roots[1] - max_wav)) / max_wav * c_km_s

        if resolution is None:
            return fwhm[0] * units.km / units.s
        else:
            print(
                "[INFO] FWHM is corrected for the provided "
                "resolution of R={}".format(resolution)
            )
            resolution_km_s = c_km_s / resolution
            return np.sqrt(fwhm**2 - resolution_km_s**2)[0] * units.km / units.s


def get_nonparametric_measurements(
    input_spec, line_rest_wavel, redshift, disp_range=None
):
    """
    ...
    flux_fraction = cumtrapz(input_spec.fluxden, input_spec.dispersion, initial=0)
    ...
    v_res_at_line = np.abs(velocity_disp[idx_50 - 1] - velocity_disp[idx_50 + 1]) / 2.0
    ...
    z_v50 = wave_v50.value / line_rest_wavel - 1.0
...
def calc_lwav_from_fwav(fluxden, redshift, cosmology):
    ...
    return (fluxden * lum_distance**2 * 4 * np.pi * (1.0 + redshift)).decompose(
        bases=[units.erg, units.s, units.AA]
    )


def calc_integrated_luminosity(input_spec, redshift, cosmology, disp_range=None):
    ...
    rest_dispersion = input_spec.dispersion / (1.0 + redshift)
    rest_fluxden = calc_lwav_from_fwav(
        input_spec.fluxden * input_spec.fluxden_unit, redshift, cosmology
    )
    rest_frame_spec = sod.SpecOneD(
...
def calc_absolute_mag_from_fluxden(
    fluxden, dispersion, cosmology, redshift, kcorrection=True, a_nu=0
):
    ...
    return calc_absolute_mag_from_apparent_mag(
        abmag, cosmology, redshift, kcorrection, a_nu
    )


def calc_absolute_mag_from_fluxden_alt(
    fluxden, dispersion, cosmology, redshift, kcorrection=True, a_nu=0
):
    """
...
def calc_absolute_mag_from_monochromatic_luminosity(l_wav, wavelength, redshift):
    ...
    :rtype: astropy.units.Quantity
    """
    l_nu = l_wav * (wavelength) ** 2 / const.c / 4 / np.pi
...
def calc_absolute_mag_from_apparent_mag(
    appmag, cosmology, redshift, kcorrection=True, a_nu=0
):
...
def k_correction_pl(redshift, a_nu):
    ...
    """
    # Hogg 1999, eq. 27
    return -2.5 * (1.0 + a_nu) * np.log10(1.0 + redshift) * units.mag
