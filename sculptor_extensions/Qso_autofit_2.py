...
qso_contfe_CIV_Shen11 = {
    ...
    "rest_frame": True,
    "mask_ranges": [[1340, 1375], [1445, 1465], [1700, 1705]],          ##additional window [1340, 1375] in order to get the correct continuum luminosity at 1350 for BH masses
}
...
qso_contfe_MgII_Xshooter = {
    "name": "QSO Cont. MgII Schindler2020",  # adapted from Schindler 2020 and Shen 2011 paper
    "rest_frame": True,
    "mask_ranges": [[2480, 2700], [2900, 3090]],
}
...
CIV_range = {"name": "CIV_range", "rest_frame": True, "mask_ranges": [[1465, 1600]]}
...
# Ground-based near-infrared observations are highly affected by the atmospheric aborptions in certain regions.
...
def initialise_balmer_without_hol(prefix, balmer_dict=None, median_params=None):
    ...
    comp = sccomp.FitComponent(prefix, balmer_continuum_model2)
    params = comp.create_params()
...
def setup_Hgamma_lines(spec, line_dict, narrow=False, Hgamma_broad=1, model=None):
    """
    Setup emission line model for Hgamma emission line 
    """
    if narrow:
        comp_Hgamma_narrow, pars_Hgamma_narrow = initialise_gaussian_line_model(
            "Hgamma_narrow", line_dict["Hgamma"]["narrow"]["params"]
        )
        model.add_component(comp_Hgamma_narrow)
        model.add_parameters(pars_Hgamma_narrow)

    for i in range(Hgamma_broad):
        comp_Hgamma_broad, pars_Hgamma_broad = initialise_gaussian_line_model(
            f"Hgamma_broad_{string.ascii_lowercase[i]}",
            line_dict["Hgamma"]["broad"]["params"],
        )
        model.add_component(comp_Hgamma_broad)
        model.add_parameters(pars_Hgamma_broad)

def setup_OI_lines(spec, line_dict, OI_broad=1, model=None):
    """
    Setup emission line model for OI emission line 
    """
    for i in range(OI_broad):
        comp_OI_broad, pars_OI_broad = initialise_gaussian_line_model(
            f"OI_broad_{string.ascii_lowercase[i]}",
            line_dict["OI"]["broad"]["params"],
        )
        model.add_component(comp_OI_broad)
        model.add_parameters(pars_OI_broad)

def setup_CIV_lines(
    spec, line_dict, narrow=False, CIV_broad=1, CIV_broadshifted=1, model=None
):
    """
    Setup emission line model for CIV emission line (1549.06 Å) using multiple broad and narrow gaussian components.
    CIV line paramters are constarined using the priors set in the emission line dictionary provided.
    Generally, CIV line, a high ionization line, is usually well described using upto three broad gaussians(Shen et al. 2011)
    which account for AGN driven outflows and winds resulting in asymmetric line profiles.

    :param spec: input spectrum
    :type spec: sculptor.speconed.SpecOneD
    :param line_dict: dictionary containing emission line parameters that contains necessary parameters to fit the CIV emission line
    such as "flux", "redshift", "fwhm_km_s", "cen", "vel_shift". In this dictionary, each component should have "initial_value", "vary" and "prior"
    which provides information on initial value of the parameter, whether the parameter should be varied, and
    prior to be used to contrain the parameter, respectively.

    :type line_dict: dictionary
    :param narrow: fit narrow component for CIV line
    :type narrow: Boolean
    :param CIV_broad: number of broad components to fit
    :type CIV_broad: int
    :param model: Model to fit
    """
    if narrow:
        comp_CIV_narrow, pars_CIV_narrow = initialise_gaussian_line_model(
            "CIV_narrow", line_dict["CIV"]["narrow"]["params"]
        )
        model.add_component(comp_CIV_narrow)
        model.add_parameters(pars_CIV_narrow)

    for i in range(CIV_broad):
        comp_CIV_broad, pars_CIV_broad = initialise_gaussian_line_model(
            f"CIV_broad_{string.ascii_lowercase[i]}",
            line_dict["CIV"]["broad"]["params"],
        )
        model.add_component(comp_CIV_broad)
        model.add_parameters(pars_CIV_broad)

    for i in range(CIV_broadshifted):
        comp_CIV_broadshifted, pars_CIV_broadshifted = initialise_gaussian_line_model(
            f"CIV_broadshifted_{string.ascii_lowercase[i]}",
            line_dict["CIV"]["broadshifted"]["params"],
        )
        model.add_component(comp_CIV_broadshifted)
        model.add_parameters(pars_CIV_broadshifted)
...
def setup_iron_template_VW01_UV(prefix, fe_dict=None, median_params=None):
    ...
    comp, params = initialise_Fe_fast(
        prefix, Fe_dispersion, Fe_fluxden, fe_dict, median_params
    )
    return comp, params


def setup_iron_template_V01_FUV(prefix, fe_dict=None, median_params=None):
    """
    Setup UV part of the Vestergaard et al. 2001 template that coveres the far UV region between 1074-2200 Å.
    ...
    Note: Flux values from the iron template are in units of 1e-17 erg/s/cm2
    """
    disp_range = [1074, 2200]

    template = np.loadtxt(datadir + "iron_templates/" + "Fe_UVtemplt_A.asc")
    Fe_dispersion = template[:, 0]
    MgII_disp_mask = (Fe_dispersion >= disp_range[0]) & (Fe_dispersion <= disp_range[1])
...
def setup_iron_template_V01_CIV(prefix, fe_dict=None, median_params=None):
    """
    Setup UV part of the Vestergaard et al. 2001 template that coveres the UV region around CIV emission line between 1440-1710 Å.
...
def setup_iron_template_Park22_OPT_Hbeta(prefix, fe_dict=None, median_params=None):
    """
    Setup the PArk et al. 2022 iron template covering the region around Hbeta between 4000 - 5600 Å.
    The region is chosen from Shen et al. 2011 continnum windows.
    ...
    template = np.loadtxt(datadir + "iron_templates/" + "Fe_OPT_Hbeta_Park22.txt")
    Fe_dispersion = template[:, 0]
    Fe_fluxden = template[:, 1]* 1.0e17
...
def get_line_prop_MgII(df, redshift, param_name_dict, save_dir, source, fit_name):
    ...
    # int_flux_col = param_name_dict.get("int_flux")
    ...
    for idx, row in df.iterrows():
        for key in ["_median", "_low", "_upp"]:
            ...
            if fwhm_col and lum_col:
                ...
                if fwhm_key in df.columns and lum_key in df.columns:
                    ...
                    if not (np.isnan(fwhm) or np.isnan(lum)):
                        if "Tsuzuki" in fit_name or fit_name == "Vest":
                            BH_mass_s11 = qso.se_bhmass_mgii_s11_fwhm(
                                fwhm, cont_lwav=lum, cont_wav=3000 * u.AA
                            )
                        if fit_name == "Vest":
                            BH_mass_vo09 = qso.se_bhmass_mgii_vo09_fwhm(
                                fwhm, cont_lwav=lum, cont_wav=3000 * u.AA
                            )
                        if "Tsuzuki" not in fit_name and fit_name != "Vest":
                            print(
                                f"[INFO] Unknown fit_name '{fit_name}' — skipping BH mass calc."
                            )
                            BH_mass_s11 = [np.nan * u.Msun]
                            BH_mass_vo09 = [np.nan * u.Msun]

                        df.at[idx, "MgII_BH_mass_3000_s11" + key] = BH_mass_s11[0].value
                        ...
                        Edd_ratio = qso.calc_eddington_ratio(L_bol[0], BH_mass_s11[0])
                        df.at[idx, "Eddington_ratio_3000_s11" + key] = Edd_ratio.value

                        # Eddington luminosity
                        Edd_lum = qso.calc_eddington_luminosity(BH_mass_s11[0])
                        df.at[idx, "Edd_lum_MgII_3000_s11" + key] = Edd_lum.value

                        if fit_name == "Vest": 
                            df.at[idx, "MgII_BH_mass_3000_vo09" + key] = BH_mass_vo09[0].value
                            Edd_ratio = qso.calc_eddington_ratio(L_bol[0], BH_mass_vo09[0])
                            df.at[idx, "Eddington_ratio_3000_vo09" + key] = Edd_ratio.value

                            # Eddington luminosity
                            Edd_lum = qso.calc_eddington_luminosity(BH_mass_vo09[0])
                            df.at[idx, "Edd_lum_MgII_3000_vo09" + key] = Edd_lum.value
            ...
            # if fe_amp_col and int_flux_col:
            #     fe_key = fe_amp_col + key
            #     flux_key = int_flux_col + key
            #     if fe_key in df.columns and flux_key in df.columns:
            #         try:
            #             if "Tsuzuki" in fit_name:
            #                 MgII_disp = [2200, 3500]
            #                 template = np.loadtxt(datadir + "iron_templates/" + "Tsuzuki06.txt")
            #                 Fe_dispersion = template[:, 0]
            #                 MgII_disp_mask = (Fe_dispersion >= MgII_disp[0]) & (Fe_dispersion <= MgII_disp[1])
            ...
            #             Fe_spec = sod.SpecOneD(dispersion=Fe_dispersion,
            #                                    fluxden=Fe_fluxden * row[fe_key],
            #                                    dispersion_unit = dispersion_unit, fluxden_unit=fluxden_unit)
            #             fe_int_flux = scana.get_integrated_flux(Fe_spec, disp_range = [2200 ,3090 ])
            #             fe_flux_ratio = fe_int_flux.value / row[flux_key]
            #             df.at[idx, "FeII_over_MgII" + key] = fe_flux_ratio
            #             df.at[idx, "FeII_flux" + key] = fe_int_flux.value

            #             ##
            #             corr_MgII_ew = row[flux_key] * ((10 ** (-0.55)/ Edd_ratio.value)**-0.30)
            #             corr_fe_flux_ratio = fe_int_flux.value / corr_MgII_ew
            #             df.at[idx, "corr_FeII_over_MgII" + key ] = corr_fe_flux_ratio
            #             ##
...
def get_line_prop_Hbeta_OIII(
    ...
    for idx, row in df.iterrows():
        for key in ["_median", "_low", "_upp"]:
        # Hbeta_based_mass, continuum luminosity and Eddington ratio
            if fwhm_col and lum_col:
                fwhm_key = fwhm_col + key
                lum_key = lum_col + key
            ...
            #     if fe_key in df.columns and flux_key in df.columns:
            #         try:
            #             Hbeta_disp = [4434, 4684]
            #             template = np.loadtxt(datadir + "iron_templates/" + "Fe_OPT_BR92_linear.txt")
            #             Fe_dispersion = template[:, 0]
            #             Hbeta_disp_mask = (Fe_dispersion >= Hbeta_disp[0]) & (Fe_dispersion <= Hbeta_disp[1])
            #             Fe_dispersion = Fe_dispersion[Hbeta_disp_mask]
            #             Fe_fluxden = template[:, 1][Hbeta_disp_mask] * 1.0e17
            ...
            #                                    fluxden=Fe_fluxden * row[fe_key],
            #                                    dispersion_unit = dispersion_unit, fluxden_unit=fluxden_unit)
            #             fe_int_flux = scana.get_integrated_flux(Fe_spec, disp_range = [4434, 4684])
            #             fe_flux_ratio = fe_int_flux.value / row[flux_key]

            #             df.at[idx, "fe_int_flux" + key] = fe_int_flux.value
            #             df.at[idx, "r_FeII_Hbeta" + key] = fe_flux_ratio

            #         except Exception as e:
            #             print(f"[INFO] FeII_Hbeta failed at row {idx}: {e}")

    return df


def get_civ_line_prop(
    df, redshift, param_name_dict, save_dir, source, model, flat_samples
):
    fwhm_col = param_name_dict.get("fwhm")
    lum_col = param_name_dict.get("luminosity")
    ...
    for idx, row in df.iterrows():
        for key in ["_median", "_low", "_upp"]:
            # CIV_based_mass, continuum luminosity and Eddington ratio
            if fwhm_col and lum_col:
                fwhm_key = fwhm_col + key
                lum_key = lum_col + key

                if fwhm_key in df.columns and lum_key in df.columns:
                    fwhm_civ = np.float64(row[fwhm_key])
                    cont_lwave_1350 = np.float64(row[lum_key])

                    if not (np.isnan(fwhm_civ) or np.isnan(cont_lwave_1350)):
                        civ_BH_mass_1350 = qso.se_bhmass_civ_vp06_fwhm(
                            civ_fwhm=fwhm_civ,
                            cont_lwav=cont_lwave_1350,
                            cont_wav=1350 * u.AA,
                        )
                        L_bol_1350 = qso.calc_bolometric_luminosity(
                            cont_lwav=cont_lwave_1350, cont_wav=1350 * u.AA
                        )
                        Edd_lum_civ_1350 = qso.calc_eddington_luminosity(
                            civ_BH_mass_1350[0]
                        )
                        Eddington_ratio_1350 = qso.calc_eddington_ratio(
                            L_bol_1350[0], civ_BH_mass_1350[0]
                        )

                        df.at[idx, "civ_BH_mass_1350" + key] = civ_BH_mass_1350[0].value
                        df.at[idx, "Edd_lum_civ_1350" + key] = Edd_lum_civ_1350.value
                        df.at[idx, "Eddington_ratio_1350" + key] = (
                            Eddington_ratio_1350.value
                        )
                        df.at[idx, "L_bol_1350" + key] = L_bol_1350[0].value
                    else:
                        print(
                            f"[INFO] NaN in '{fwhm_key}' or '{lum_key}' at row {idx} — skipping BH mass and Eddington calculations."
                        )
            ...
            if z_col:
                ...
                if z_key in df.columns:
                    try:
                        delta_v_civ = qso.calc_velocity_shifts(
                            row[z_key], z_sys=redshift, relativistic=True
                        )
                        df.at[idx, "delta_v_civ" + key] = delta_v_civ.value
                    except Exception as e:
                        print(
                            f"[INFO] Failed to calculate velocity shift for '{z_key}' at row {idx}: {e}"
                        )
    ...
    # df_path = os.path.join(save_dir, f"{source}_results_Hbeta_OIII.csv")
    # df.to_csv(df_path, index=False, float_format="%.4e")
    return df
...
##changes from Francesco's faster FeII
from scipy.ndimage import gaussian_filter1d
def make_template_model_nopickle(
    templ_disp,
    templ_fluxden,
):
    # --- Precompute fixed log-wavelength grid ---
    template_spec = sod.SpecOneD(
        dispersion=templ_disp,
        fluxden=templ_fluxden,
    )
    spec_log = template_spec.copy()
    spec_log._to_log_wavelength()

    log_disp = np.linspace(
        spec_log.dispersion.min(),
        spec_log.dispersion.max(),
        num=len(spec_log.dispersion),
    )
    # Interpolate fluxden onto uniform log grid once
    log_fluxden = np.interp(log_disp, spec_log.dispersion, spec_log.fluxden)

    # Pixel scale in log-lambda (uniform, so constant)
    dloglam = log_disp[1] - log_disp[0]
    c_kms = const.c.to("km/s").value

    def template_model(x, redsh, amp, fwhm, intr_fwhm, **kwargs):
        convol_fwhm = np.sqrt(fwhm**2 - intr_fwhm**2)

        sigma_log = (convol_fwhm / c_kms) / (2 * np.sqrt(2 * np.log(2)))
        sigma_pix = sigma_log / dloglam

        conv = gaussian_filter1d(
            log_fluxden,
            sigma=sigma_pix,
            mode="constant",
            cval=0.0,
        ...
        # Undo the fluxden *= dispersion_linear from _to_log_wavelength
        conv /= np.exp(log_disp)

        # Normalization
        conv /= (2 * np.sqrt(2 * np.log(2))) ** 2

        lin_disp = np.exp(log_disp) * (1.0 + redsh)
        return np.interp(x, lin_disp, conv, left=0, right=0) * amp

    return template_model

def initialise_Fe_fast(prefix, Fe_dispersion, Fe_fluxden, Fe_dict=None, median_params=None):
    """
    Setup an iron template using parameters defined in a dictionary. These parameters include "amp",
    "fwhm", "redshift", "intr_fwhm", "templ_disp" and "templ_fluxden".
    In this dictionary, each component should have "initial_value", "vary" and "prior"
    ...
    :return: iron template component and parameters
    """
    fast_model = make_template_model(Fe_dispersion, Fe_fluxden)
    comp = sccomp.FitComponent(prefix, fast_model)
    params = comp.create_params()
...
class _FakeCode:
    """Mimics the two attributes FitComponent needs from a real code object."""

    def __init__(self, co_argcount, co_varnames):
...
class TemplateModel:
    """Picklable replacement for the make_template_model closure."""

    def __init__(self, templ_disp, templ_fluxden):
        template_spec = sod.SpecOneD(
            dispersion=templ_disp,
            fluxden=templ_fluxden,
        )
        ...
        self.log_disp = np.linspace(
            spec_log.dispersion.min(),
            spec_log.dispersion.max(),
            num=len(spec_log.dispersion),
        )
        self.log_fluxden = np.interp(
            self.log_disp, spec_log.dispersion, spec_log.fluxden
        )
        self.dloglam = self.log_disp[1] - self.log_disp[0]
        self.c_kms = const.c.to("km/s").value

        # Fake the same __code__ shape the original closure had:
        # template_model(x, redsh, amp, fwhm, intr_fwhm, **kwargs)
        self.__code__ = _FakeCode(
            co_argcount=5,
            co_varnames=("x", "redsh", "amp", "fwhm", "intr_fwhm"),
        )
        self.__name__ = "template_model"

    def __call__(self, x, redsh, amp, fwhm, intr_fwhm, **kwargs):
        convol_fwhm = np.sqrt(fwhm**2 - intr_fwhm**2)

        sigma_log = (convol_fwhm / self.c_kms) / (2 * np.sqrt(2 * np.log(2)))
        sigma_pix = sigma_log / self.dloglam
        ...
        conv /= np.exp(self.log_disp)
        conv /= (2 * np.sqrt(2 * np.log(2))) ** 2

        lin_disp = np.exp(self.log_disp) * (1.0 + redsh)
        return np.interp(x, lin_disp, conv, left=0, right=0) * amp
...
def make_template_model(templ_disp, templ_fluxden):
    return TemplateModel(templ_disp, templ_fluxden)


# =========================================================================== #

_H, _K, _C = const.h.cgs.value, const.k_B.cgs.value, const.c.cgs.value
_C_AA = const.c.to('AA/s').value

_SHAPE_CACHE = {}
_CACHE_MAX = 4


def balmer_continuum_model2(x, z, amp_be, Te, tau_be, lambda_be):
    """Dietrich 2003 Balmer continuum."""
    key = (x.tobytes(), float(z), float(Te), float(tau_be), float(lambda_be))
    shape = _SHAPE_CACHE.get(key)

    if shape is None:
        xr = x / (1. + z)
        shape = np.zeros_like(xr, dtype=float)
        m = xr < lambda_be                     # zero above the Balmer edge
        if m.any():
            xm = xr[m]
            nu = _C_AA / xm
            b_nu = 2.*_H*nu**3 / _C**2 / np.expm1(_H*nu / (_K*Te))
            shape[m] = (b_nu * _C_AA / xm**2
                        * (1. - np.exp(-tau_be*(xm/lambda_be)**3)) * 1e-20)
        if len(_SHAPE_CACHE) >= _CACHE_MAX:
            _SHAPE_CACHE.clear()
